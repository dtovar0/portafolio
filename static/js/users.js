...
async function refreshPicklistAreas(prefix, selectedNames = []) {
    try {
        ...
        const available = areas.filter(a => !selectedNames.includes(a.name) || selectedNames.includes(a));
        const assigned = areas.filter(a => selectedNames.includes(a.name) || selectedNames.includes(a));
...
function updateUserAreasHiddenInput(prefix) {
    const selected = Array.from(document.querySelectorAll(`#${prefix}SelectedList .card-name`)).map(el => el.textContent);
    const input = document.getElementById(prefix === 'editUser' ? 'editSelectedUserAreasInput' : 'selectedUserAreasInput');
...
function createAreaCardModern(area, isSelected, prefix) {
    ...
    card.innerHTML = `
        <div class="card-icon" style="background: ${area.color || getAreaColor(area.name)}">
            ${iconsMap[area.icon] || iconsMap['box']}
        </div>
        <div class="card-info">
            <span class="card-name">${area.name}</span>
            <span class="card-meta">${area.description || 'Área de trabajo'}</span>
        </div>
        <div class="card-action-icon">
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.addEventListener('click', (event) => {
        ...
        if (action === 'users-toggle-access') return toggleUserAccess(trigger.dataset.platformName, trigger.dataset.isSelected === 'true');
        if (action === 'users-toggle-area') {
            const areaName = trigger.dataset.areaName;
            const isEdit = trigger.dataset.listId.includes('edit');
            const area = (window.__areaData || []).find(a => a.name === areaName) || { name: areaName };
            return toggleUserAreaPremium(area, trigger.dataset.listId.includes('Available'), isEdit ? 'editUser' : 'addUser');
        }
...
async function saveNewUser(e) {
    ...
    fd.append('areas', selectedAreas);
    fd.append('status', document.getElementById('addUserStatusToggle').checked ? 'Activo' : 'Deshabilitado');
...
async function saveUserChanges(e) {
    ...
    const areasVal = document.getElementById('editSelectedUserAreasInput').value;
    const areas = JSON.parse(areasVal || '[]');
...
async function adminUserAccess(userId) {
    ...
    availList.innerHTML = '<div class="text-center p-2"><i class="fas fa-spinner fa-spin"></i> Cargando...</div>';
...
function createPlatformAccessCard(p, isSelected) {
    ...
    card.innerHTML = `
        <div class="card-icon" style="background: ${p.bg_color || 'var(--bg-hover)'}; color: ${p.text_color || 'white'}">
            <i class="fas fa-layer-group"></i>
        </div>
        <div class="card-info">
            <span class="card-name">${p.name}</span>
            <span class="card-meta">Área: ${p.area_name || 'General'}</span>
        </div>
        <div class="card-action-icon">
            <i class="fas ${isSelected ? 'fa-times' : 'fa-plus'}"></i>
        </div>
