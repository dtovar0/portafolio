// Requires: window.__platformData, window.__areaData, window.__allUsers, window.__urls set inline
    ...
    function renderAreaGrid() {
        ...
        pageData.forEach(area => {
            ...
            const accentColor = is_disabled ? '#ef4444' : 'var(--primary-color)';

            card.innerHTML = `
                <div style="display: flex; align-items: center; gap: 1.25rem; margin-bottom: 1.25rem;">
                    <div class="area-card-icon" style="background: ${hexToRGBA(accentColor, 0.12)}; color: ${accentColor};">
                ...
                <div class="health-bar-container" style="height: 6px; background: #f1f5f9; border-radius: 3px; overflow: hidden; margin-bottom: 0.75rem;">
                    <div class="health-bar" style="width: 100%; height: 100%; background: ${accentColor}; border-radius: 3px;"></div>
                </div>
                <div style="display: flex; justify-content: space-between; align-items: center; font-size: 0.75rem;">
                    <span style="color: ${accentColor}; font-weight: 600;">
    ...
    function renderPlatformsTable(data) {
        ...
        if (data.length === 0) {
            const isSearch = document.getElementById('platformSearchSub')?.value.trim() !== "";
    ...
    function filterPicklist(input, listId) {
        ...
        const items = list.getElementsByClassName('picklist-card-premium');
        for (let item of items) {
            const name = item.querySelector('.card-name')?.textContent || item.textContent;
    ...
    function updatePicklist() {
        ...
        // Keep IDs numeric to avoid string/number mismatch when toggling by click.
        ...
        allUsers.forEach(u => {
            ...
            const isActive = selectedUserIds.includes(Number(u.id));
            card.innerHTML = `
                <span>${u.name}</span>
    ...
    function moveItem(userId) {
        ...
        const index = selectedUserIds.indexOf(normalizedUserId);
        if (index > -1) {
            selectedUserIds.splice(index, 1);
        } else {
            selectedUserIds.push(normalizedUserId);
        }
        updatePicklist();
    }
