...
var allAreasData = Array.isArray(window.__areaData) ? window.__areaData : [];
var Toast = window.Toast || (window.Swal ? Swal.mixin({
    toast: true,
    position: 'bottom-end',
...
window.changeAreaStep = function(mode, step) {
    ...
    // If this view is using the simplified modal (no stepper), skip wizard logic.
    if (!s1 || !s2 || !s3 || !progress || !btnBack || !btnNext || !btnSubmit) {
        return;
    }
...
function toggleAreaModal() {
    ...
    // Reset to Step 1 if wizard exists.
...
function saveNewArea() {
    ...
    const statusToggle = document.getElementById('addAreaStatusToggle');
    const status = statusToggle ? (statusToggle.checked ? 'Activo' : 'Deshabilitado') : 'Activo';
    formData.set('status', status);
    const colorInput = document.getElementById('addAreaColorInput');
    formData.set('color', colorInput ? colorInput.value : colorsPalette[0]);
    
    // Add users
    const selectedUsersInput = document.getElementById('selectedAreaUsersInput');
    const userIds = JSON.parse((selectedUsersInput && selectedUsersInput.value) || '[]');
...
function updateAreaUsersHiddenInput(mode) {
    ...
    const usersData = Array.isArray(window.__userData) ? window.__userData : [];
    const ids = names.map(name => {
        const u = usersData.find(user => user.name === name);
        return u ? u.id : null;
    }).filter(id => id !== null);
...
function editSelectedArea() {
    ...
    // Icon (radio-based picker)
    const editIconInput = document.querySelector(`#editIconGroup input[name="iconEdit"][value="${area.icon || 'box'}"]`);
    if (editIconInput) {
        editIconInput.checked = true;
    }

    // Color
    const colorInput = document.getElementById('editAreaColorInput');
    colorInput.value = area.color || colorsPalette[0];
    document.querySelectorAll('#editColorGroup .color-option').forEach(opt => {
        opt.classList.remove('active');
        if (opt.dataset.color === area.color) opt.classList.add('active');
    });
...
function saveEditedArea() {
    ...
    const selectedIcon = document.querySelector('#editIconGroup input[name="iconEdit"]:checked');
    formData.append('icon', selectedIcon ? selectedIcon.value : 'box');
...
function renderAreasTable() {
    ...
    pageData.forEach(area => {
        ...
        tr.innerHTML = `
            <td class="col-cb">
                <input type="checkbox" class="area-checkbox area-cb" value="${area.id}" data-id="${area.id}" style="cursor: pointer; width: 16px; height: 16px;">
            </td>
            <td style="width: 60px;">
...
function updateActionButtons() {
    const checkedCount = document.querySelectorAll('.area-checkbox:checked, .area-cb:checked').length;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.addEventListener('click', (event) => {
        ...
        if (action === 'areas-select-all') {
            const shouldCheck = !!trigger.checked;
            document.querySelectorAll('.area-checkbox, .area-cb').forEach((cb) => {
    ...
    document.addEventListener('change', (event) => {
        if (event.target.classList.contains('area-checkbox') || event.target.classList.contains('area-cb')) {
            updateActionButtons();
        }
